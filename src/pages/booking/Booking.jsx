...
function Booking() {
  ...
  const filtered = useMemo(() => {
    return bookingDummyData
      .filter((b) => (status === "All" ? true : (b.status || "").toLowerCase() === status.toLowerCase()))
      .filter((b) =>
        service === "All"
          ? true
      ...
      .filter((b) =>
        payment === "All" ? true : (b.paymentStatus || b.payment || "").toLowerCase() === payment.toLowerCase()
      )
      .filter((b) => {
  ...
  const handleAssignCleaner = (booking) => {
    const isResidential = (booking.cleaningDetails?.serviceType || "").toLowerCase().includes("residential");
    if (isResidential && booking.scheduling?.assignedCleaner) {
      toast.info("Residential bookings allow only one cleaner.");
      return;
    }
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <select
              ...
              className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm"
            >
              {["All", "Residential", "Commercial", "Post-Construction", "Deep"].map((s) => (
                <option key={s} value={s}>
                  {s}
                </option>
              ))}
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {paginated.map((b) => {
                ...
                const paymentStatus = b.paymentStatus || b.payment || "Unpaid";
                return (
                  <tr key={b.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-gray-700">
                      ...
                      <div className="text-xs text-gray-400">Contract: {b.customerInfo?.address}</div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        {paymentStatus}
                        <span className="text-gray-800">${b.pricing?.totalPrice?.toLocaleString()}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
            ...
            return (
              <div key={b.id} className="rounded-xl border border-gray-200 p-4 shadow-sm">
                ...
                <div className="mt-3 space-y-2 text-sm text-gray-700">
                  ...
                  >
                    {paymentStatus} · ${b.pricing?.totalPrice?.toLocaleString()}
                  </div>
                </div>
